...
def main(cfg):
    ...
    train_dataset = CustomDataset(
        df=train_df, cfg=cfg, aug=cfg.train_transforms
    )
    ...
    for epoch in range(cfg.epochs):
        ...
        if val_metric > best_metric:
            print(
                f"SAVING CHECKPOINT: val_metric {best_metric:.5} -> {val_metric:.5}"
            )
...
def run_train(
    ...
    for itr in progress_bar:
        batch = next(tr_it)
        inputs, labels = batch["image"].to(cfg.device), batch[
            "label"
        ].float().to(cfg.device)
        iteration += 1
...
def run_eval(model, val_dataloader, cfg, writer, epoch):
    ...
    auc = roc_auc_score(all_labels, all_outputs)
    all_outputs = (
        (np.array(all_outputs) > cfg.clf_threshold).astype(np.int8).tolist()
    )
    try:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-s", "--seed", type=int, default=20230101, help="seed"
    )
    parser.add_argument(
        "-w", "--weights", default=None, help="the path of weights"
    )
